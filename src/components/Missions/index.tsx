  ...
  return (
    ...
    >
      {missions.map(({ title, duration, text, link }: Mission, idx: number) => (
        <div
          class="flex sm:flex-col lg:flex-row sm:items-center lg:justify-between mb-8 lg:items-baseline"
          key={idx}
        >
          <div class="relative z-10 sm:w-max sm:mb-10 lg:w-1/4 text-center">
            <a
              href={link}
              target="_blank"
            ...
            {idx != missions.length - 1 && (
              <div
                class={`absolute left-1/2 sm:h-8 md:h-8 lg:h-20 xl:h-16 border ${themeValue == "light" ? "border-blue-300" : "border-green-300"} transform md:translate-y-1 lg:translate-y-2`}
              ></div>
            )}
          </div>
          <div class="lg:w-9/12">
            <p
              class={`font-body sm:text-center sm:px-4 md:px-8 md:text-sm lg:text-left lg:text-base lg:pr-10 ${themeValue == "light" ? "text-black" : "text-slate-200"}`}
            >
              {text}
            </p>
          </div>
