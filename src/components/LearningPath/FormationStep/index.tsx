...
const FormationStep: FunctionComponent<{
  ...
  return (
    ...
    >
      <p
        class={`font-display font-bold text-2xl ${themeValue == "light" ? "text-black" : "text-slate-200"} ${selected && "translate-x-4"} transition-transform group-hover:transform group-hover:translate-x-4`}
      >
        {title}
      </p>
      <p
        class={`font-body ${themeValue == "light" ? "text-slate-500" : "text-slate-300"} italic`}
      >
        {description}
      </p>
      <p class="font-body">{duration}</p>
    </div>
  );
};
